    ...
    try:
        ...
        #recuperar los tiempos unicos por cada agente
            intervalos = db.query(models.Administered_features.timer,models.Administered_features.id_adminis).filter(
                models.Administered_features.id_agent == f'{agent.id_agent}').distinct().all()
            ...
            for inter in intervalos:
                ...
                features = db.query(models.Administered_features.oid,models.Administered_features.id_adminis).filter(
                    ...
                    models.Administered_features.id_agent == f'{agent.id_agent}').all()
...
async def CreatorTask(elements):
    ...
    for agent in elements:
        for TIME,OIDS,IDF in zip(agent['TIMES'],agent['OIDS'],agent['IDF']):
            ...
            tasks.append(asyncio.create_task(Get_SNMP(TIME=TIME,OIDS=oid,ID=agent['ID'],IDF=IDF,IP=agent['IPS'])))
